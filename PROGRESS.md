...
- **Screenshot button "截屏失败"**: Three-layer fix — (1) added `display-capture` to `setPermissionRequestHandler`/`setPermissionCheckHandler` in `electron/main.cjs:458` (was previously denied), (2) added `desktopCapturer.getSources()` IPC handler (`screenshot:capture`) that works without macOS screen recording permission, (3) chat.js falls back to `getLatestSystemScreenshot` (clipboard) before attempting browser `getDisplayMedia`.
