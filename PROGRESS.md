...
- **Screenshot button "截屏失败"**: `getDisplayMedia` now tried first (triggers native macOS permission dialog), then Electron IPC (`desktopCapturer` + `clipboard.readImage()`), then clipboard. `screenshot:open-settings` IPC handler opens System Settings > Screen Recording via `shell.openExternal`. Toast shows permission guidance with a clickable "開啟設定" button (`pointer-events: auto` on `.screenshot-settings-btn`).
