...
- **Screenshot button changed to image upload**: Replaced entire screenshot flow (getDisplayMedia/desktopCapturer/clipboard) with a hidden `<input type="file" accept="image/*">`. Button icon changed from 📷 to 🖼️, title changed to "上传图片（AI 會以視覺分析圖片）". Removed unused Electron IPC (`screenshot:capture`, `screenshot:open-settings`, `display-capture` permission, `desktopCapturer` import, `.screenshot-settings-btn` CSS).
