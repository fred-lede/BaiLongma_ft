...
// 跟踪本轮 generate_image 生成的图片 URL，自动注入到 send_message 的内容中
const pendingGenImageUrls = new Set()
let pendingGenImageFromCurrentTurn = false  // 标记 pendingGenImageUrls 是否来自本轮调用
...
async function executeToolUnchecked(name, args, context = {}) {
  try {
    ...
    switch (name) {
      ...
      case 'generate_image': {
        const genResult = await execGenerateImage(args)
        // 从工具结果中提取图片 URL，供稍后的 send_message 自动注入
        pendingGenImageUrls.clear()
        pendingGenImageFromCurrentTurn = true
        const urlMatches = String(genResult).match(/(\/media\/chat\/[^\s)\]]+)/g)
        if (urlMatches) for (const url of urlMatches) pendingGenImageUrls.add(url)
        return genResult
      }
...
//   - reason: 失败时给 LLM 的提示
// send_message：注入 pendingGenImageUrls 后委托 deliverMessage
// 只有当消息不包含图片时才注入；如果消息已有图片则跳过，避免重复
// 只有当 generate_image 是本轮调用的才替换假的图片 URL（避免用上一轮的旧图）
async function execSendMessage({ target_id, content = '', channel = 'AUTO', image_path, media_path }, context = {}) {
  const contentHasImageUrl = content && /!\[.*?\]\(\/media\/chat\/([^)\s]+)\)/.test(content)
  if (!media_path && !image_path && pendingGenImageUrls.size > 0 && !contentHasImageUrl && pendingGenImageFromCurrentTurn) {
    // 本轮调用了 generate_image，内容没有图片 — 注入 pending URL
    content += '\n\n' + [...pendingGenImageUrls].map(u => `![generated image](${u})`).join('\n')
    pendingGenImageUrls.clear()
    pendingGenImageFromCurrentTurn = false
  } else if (contentHasImageUrl && pendingGenImageUrls.size > 0 && pendingGenImageFromCurrentTurn) {
    // 本轮调用了 generate_image，内容包含图片 — 检查是否真实存在，如果模型幻觉了 URL 则用真实的替换
    const fakeMatches = String(content).matchAll(/!\[.*?\]\(\/media\/chat\/([^)\s]+)\)/g)
    let newContent = content
    for (const match of fakeMatches) {
      const url = match[0]
      const filename = match[1]
      const fakePath = path.join(paths.mediaDir, path.basename(filename))
      if (!fs.existsSync(fakePath)) {
        const realUrl = [...pendingGenImageUrls][0]
        if (realUrl) {
          console.log(`[execSendMessage] 检测到模型幻觉的图片 URL: ${url}，替换为真实 URL: ${realUrl}`)
          newContent = newContent.replace(url, `![generated image](${realUrl})`)
        }
      }
    }
    ...
    // generate_image 不是本轮调用的 — 清空任何旧的 pending URLs，避免误用
    pendingGenImageUrls.clear()
    pendingGenImageFromCurrentTurn = false
  }
  return await deliverMessage({ target_id, content, channel, image_path, media_path }, context)
}
