...
- **Screenshot button "截屏失败"**: (1) `display-capture` added to Electron permission handlers, (2) `screenshot:capture` IPC handler tries `desktopCapturer` → `clipboard.readImage()` → `screencapture` CLI (macOS, no permission needed), (3) chat.js no longer `throw` on IPC failure so all fallbacks execute.
